...
public class FbReferenceReview implements ReviewReference {
    ...
    private final Map<ReferenceBinder<?>, ValueEventListener> mBindings;
    ...
    @NonNull
    private FbDataReference.SnapshotConverter<IdableList<DataTag>> getTagsConverter() {
        return new FbDataReference.SnapshotConverter<IdableList<DataTag>>() {
            @Override
    ...
    private <T extends HasReviewId> void bindListBinder(String child,
                                                        ReferenceBinder<IdableList<? extends T>> binder,
    ...
    private void bindSizeBinder(String child, ReferenceBinder<DataSize> binder) {
        if (!mBindings.containsKey(binder)) {
            bind(mAggregate, child, binder, newSizeListener(binder, getSize()));
        }
    }

    private void bind(Firebase root, String child, ReferenceBinder<?> binder, ValueEventListener listener) {
        root.child(child).addValueEventListener(listener);
        mBindings.put(binder, listener);
    }

    private <T> void unbindSizeBinder(String child, ReferenceBinder<T> binder) {
        unbindBinder(mAggregate, child, binder);
    }

    private <T> void unbindDataBinder(String child, ReferenceBinder<T> binder) {
        unbindBinder(mData, child, binder);
    }

    private <T> void unbindBinder(Firebase root, String child, ReferenceBinder<T> binder) {
        if (mBindings.containsKey(binder)) {
    ...
    @NonNull
    private <T extends HasReviewId> ValueEventListener newListListener(final ReferenceBinder<IdableList<? extends T>> binder,
                                                                       final SnapshotConverter<IdableList<T>> converter) {
        return new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                binder.onReferenceValue(converter.convert(dataSnapshot));
            }
    ...
    @NonNull
    private <T extends HasReviewId> ValueEventListener newDataListener(final ReferenceBinder<T> binder,
    ...
    @NonNull
    private ValueEventListener newSizeListener(final ReferenceBinder<DataSize> binder,
