...
public class ReviewDeleter {
    public boolean deleteReviewFromDb(RowReview row, ReviewerDb db, TableTransactor transactor) {
        ...
        deleteFromTagsTableIfNecessary(db.getTagsTable(), db.getTagsManager(), idString, transactor);
    ...
    private void deleteFromAuthorsTableIfNecessary(ReviewerDb db, RowReview row, TableTransactor transactor) {
        String userId = row.getAuthorId();
        TableRowList<RowReview> authored
                = transactor.getRowsWhere(db.getReviewsTable(), asEntry(RowReview.USER_ID, userId));
        if (authored.size() == 0) deleteFromTable(db.getAuthorsTable(), RowAuthor.USER_ID, userId, transactor);
    }

    private void deleteCriteriaFromReviewsTable(DbTable<RowReview> reviewsTable, ColumnInfo<String> idCol, String id, TableTransactor transactor) {
        TableRowList<RowReview> rows = transactor.getRowsWhere(reviewsTable, asEntry(RowReview.PARENT_ID, id));
