...
public class SocialUploadServiceReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if(action.equals(SocialUploadService.STATUS_UPDATE)) {
            updateListenersOnStatus(intent);
        } else if(action.equals(SocialUploadService.UPLOAD_COMPLETED)){
            updateListenersOnCompletion(intent);
        }
    }

    private void updateListenersOnCompletion(Intent intent) {
        ArrayList<PublishResults> ok = intent.getParcelableArrayListExtra(SocialUploadService
                .PUBLISH_RESULTS_OK);
        ArrayList<PublishResults> notOk = intent.getParcelableArrayListExtra(SocialUploadService
    ...
    private void updateListenersOnStatus(Intent intent) {
        double percentage = intent.getDoubleExtra(SocialUploadService.STATUS_PERCENTAGE, 0.);
        PublishResults results = intent.getParcelableExtra(SocialUploadService.STATUS_RESULTS);
        for(SocialPlatformsUploaderListener listener : mListeners) {
