 ...
 */
public class EditScreenLocations extends EditScreenReviewDataImpl<GvLocationList.GvLocation> {
    private static final GvDataType<GvLocationList.GvLocation> TYPE =
            GvLocationList.GvLocation.TYPE;

    public EditScreenLocations(Context context, ReviewBuilderAdapter builder, FactoryReviewDataEditor editorFactory) {
    ...
    private static class BannerButtonAddLocation extends BannerButtonEdit<GvLocationList.GvLocation> {
        ...
        public void onAlertPositive(int requestCode, Bundle args) {
            if (requestCode == ADD_ON_MAP) {
                LaunchableUi mapUi = FactoryLaunchableUi.newLaunchable(ActivityEditLocationMap
                        .class);
                LauncherUiImpl.launch(mapUi, getActivity(), getLaunchableRequestCode(), null,
                        new Bundle());
            }
        }
    }
    ...
    private static class GridItemEditLocation extends GridItemEdit<GvLocationList.GvLocation> {
        ...
        public void onAlertPositive(int requestCode, Bundle args) {
            if (requestCode == EDIT_ON_MAP) {
                LaunchableUi mapUi = FactoryLaunchableUi.newLaunchable(ActivityEditLocationMap.class);
                LauncherUiImpl.launch(mapUi, getActivity(), getLaunchableRequestCode(), null, args);
            }
        }
    }
}
