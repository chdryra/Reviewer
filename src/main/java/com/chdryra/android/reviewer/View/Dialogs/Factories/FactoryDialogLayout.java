...
package com.chdryra.android.reviewer.View.Dialogs.Factories;
...
public class FactoryDialogLayout {
    ...
    public FactoryDialogLayout(ConfigDataUi config) {
        mConfig = config;
    }
    ...
    (GvDataType<T> dataType, AddEditLayout.GvDataAdder adder) {
        try {
            Class<? extends AddEditLayout<T>> addEditLayout = mConfig.getAddEditLayout(dataType);
    ...
    public <T extends GvData> DialogLayout<T> newLayout
        ...
        try {
            Class<? extends DialogLayout<T>> viewClass = mConfig.getViewLayout(dataType);
