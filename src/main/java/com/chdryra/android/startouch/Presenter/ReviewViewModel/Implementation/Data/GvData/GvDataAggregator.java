...
//        DataAggregator<DataAuthor> aggregator = mAggregators.newAuthorsAggregator(mParams
// .getSimilarBoolean());
...
//        DataAggregator<DataSubject> aggregator = mAggregators.newSubjectsAggregator(mParams
...
//        DataAggregator<DataTag> aggregator = mAggregators.newTagsAggregator(mParams
// .getSimilarPercentage());
...
//        DataAggregator<DataDate> aggregator = mAggregators.newDatesAggregator(mParams
// .getSimilarDate());
...
//        return aggregateCriteria(mConverter.toGvCriterionList(root.getData()),
// CriterionAggregation.SUBJECT);
//    }

    public GvCanonicalCollection<GvCriterion> aggregateCriteria(GvDataList<GvCriterion> data,
                                                                CriterionAggregation aggregation) {
        DataAggregator<DataCriterion> aggregator;
        if (aggregation == CriterionAggregation.SUBJECT) {
            aggregator = mAggregators.newCriteriaAggregatorSameSubject(mParams
                    .getSimilarPercentage());
        } else {
            aggregator = mAggregators.newCriteriaAggregatorSameSubjectRating(mParams
                    .getSimilarBoolean());
        }
...
//        DataAggregator<DataImage> aggregator = mAggregators.newImagesAggregator(mParams
// .getSimilarBoolean());
...
//    }
    ...
    public GvCanonicalCollection<GvComment> aggregateComments(IdableList<? extends DataComment>
                                                                      data) {
        DataAggregator<DataComment> aggregator = mAggregators.newCommentsAggregator(mParams
                .getSimilarPercentage());
...
//        DataAggregator<DataLocation> aggregator = mAggregators.newLocationsAggregator(mParams
// .getSimilarLocation());
...
//        DataAggregator<DataFact> aggregator = mAggregators.newFactsAggregator(mParams
// .getSimilarPercentage());
