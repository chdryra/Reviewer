 ...
 */
public class CreateListView extends CreateView {
    public CreateListView(ReviewViewAdapter<?> unexpanded, FactoryReviewView viewFactory) {
        super(viewFactory, unexpanded);
    }

    @Override
    protected ReviewView<?> createView(AdapterReviewNode<?> adapter) {
        return getViewFactory().newListView(adapter.getNode(), null);
    }
}
