...
public class FragmentReviewTags extends FragmentReviewGridAddEditDone<GVString> {
    ...
    protected void onBannerButtonClick() {
        DialogShower.show(new DialogTagAddFragment(), FragmentReviewTags.this, DATA_ADD,
                DATA_ADD_TAG, Administrator.get(getActivity()).pack(getController()));
    }

    @Override
    protected void onGridItemClick(AdapterView<?> parent, View v, int position, long id) {
        GVString tag = (GVString) parent.getItemAtPosition(position);
        Bundle args = Administrator.get(getActivity()).pack(getController());
        args.putString(TAG_STRING, tag.get());
        DialogShower.show(new DialogTagEditFragment(), FragmentReviewTags.this, DATA_EDIT,
                DATA_EDIT_TAG, args);
    }
